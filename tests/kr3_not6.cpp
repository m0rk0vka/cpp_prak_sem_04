        ...
        void printNAryItem(int arity, char sign) {
            ...
            if (sign == '+') {
                ...
                for (int i = 0; i < new_arity; ++i) {
                    temp_2.push_back(temp_1.back());
                    temp_1.pop_back();
                }
                if (new_arity > 1) {
                    temp_2.push_back(std::make_shared<poliz::AdditionItem>(new_arity));
                }
            ...
            if (sign == '*') {
                ...
                if flag {
                    vec.push_back(std::make_shared<poliz::NumberItem>(0));
                } else {
                    for (int i = 0; i < new_arity; ++i) {
                        temp_2.push_back(temp_1.back());
                        temp_1.pop_back();
    ...
    private:
        void visitIdentifier(const poliz::IdentifierItem &item) {
            if (read) {
                ...
                vec.push_back(std::make_shared<poliz::IdentifierItem>(item.name()));
            }
        }
        void visitNumber(const poliz::NumberItem &item) {
            if (read) {
                ...
                vec.push_back(std::make_shared<poliz::NumberItem>(item.value()));
            }
        }
        void visitAddition(const poliz::AdditionItem &item) {
